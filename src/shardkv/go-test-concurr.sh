#!/bin/bash
#
# From https://gist.github.com/jonhoo/f686cacb4b9fe716d5aa#file-go-test-many-sh
#
...
if ! go test -c -o $logdir/tester; then
...
finish() {
	...
	printf "Done %03d/%d; %d pass, %d failed\n\e[0m" \
		$((success+failed)) \
		"$runs" \
		"$success" \
		"$failed"
...
echo -e "$success/$logs PASS, $failed failed"
