...
func (rf *Raft) startRequestVote(serv, term, me, lastLogIndex, lastLogTerm int) bool {
	...
	if ok := rf.sendRequestVote(serv, &args, &reply); !ok || reply.Term > rf.currTerm {
...
func(rf *Raft) startAppendEntries(serv, currTerm, me, prevLogIndex, prevLogTerm, leaderCommit int,
	...
	if m >= 0 && m < rf.raftLog.Len() && m > rf.raftLog.GetCommitIndex() &&
...
func (rf *Raft) HandleAppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.Entries == nil {
		...
		DPrintf(logReplicate, "%v reset electionTimeout", rf)
	}
...
func (rf *Raft) Kill() {
	...
	rf.Lock()
	DPrintf(always, "%v is killed %v", rf, rf.raftLog)
	rf.stat = Dead
	rf.Unlock()
}
